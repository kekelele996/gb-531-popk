...
export const useSafeguardStore = defineStore('safeguards', () => {
  ...
  async function suspend(id: number, input: SuspendSafeguardInput) { const item = await safeguardApi.suspendSafeguard(id, input); await load(); return item }
  async function resume(id: number, input: ResumeSafeguardInput) { const item = await safeguardApi.resumeSafeguard(id, input); await load(); return item }
