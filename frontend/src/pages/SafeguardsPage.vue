...
import type { ResumeSafeguardInput, Safeguard, SafeguardInput, SafeguardType, SuspendSafeguardInput } from '../types/safeguard'
...
const suspendForm = reactive<SuspendSafeguardInput>({ reason: '', compensating_measures: '', planned_restore_at: '' })
const resumeForm = reactive<ResumeSafeguardInput>({ verified_at: '', evidence_note: '' })
...
const suspendedCount = computed(() => visible.value.filter((x) => x.lifecycle_state === 'suspended').length)
...
function dateLabel(value?: string | null) { return value ? new Date(value).toLocaleDateString('zh-CN') : '—' }
function stateLabel(item: Safeguard) { if (item.lifecycle_state === 'suspended') return '暂停中'; return isExpired(item) ? '已过期' : item.lifecycle_state }
...
const disablePastDate = (date: Date) => date.getTime() < Date.now() - 8.64e7
...
function openSuspend(item: Safeguard) { suspendTarget.value = item; Object.assign(suspendForm, { reason: '', compensating_measures: '', planned_restore_at: '' }); suspendDialog.value = true }
...
async function submitSuspend() { if (!suspendTarget.value) return; saving.value = true; try { await store.suspend(suspendTarget.value.id, suspendForm); suspendDialog.value = false; ElMessage.success('保护层已暂停，新评估将不再计入该层') } catch (error) { ElMessage.error(errorMessage(error)) } finally { saving.value = false } }
async function submitResume() { if (!resumeTarget.value) return; saving.value = true; try { await store.resume(resumeTarget.value.id, resumeForm); resumeDialog.value = false; ElMessage.success('保护层已恢复，后续评估将重新计入') } catch (error) { ElMessage.error(errorMessage(error)) } finally { saving.value = false } }
onMounted(refresh)
</script>

<template>
  <AppShell>
    <PageHeader eyebrow="INDEPENDENT PROTECTION REGISTER" title="保护层台账" description="核对覆盖目标、独立性键、有效性与验证期限；同一路径的重复独立性键只计一次，暂停中的措施不参与新的覆盖评估。">
    ...
    <section class="filter-bar"><div><ShieldCheck :size="16" /><el-select v-model="filterScenario" placeholder="全部偏差场景" clearable><el-option v-for="item in scenarios.items" :key="item.id" :label="scenarioLabel(item.id)" :value="item.id" /></el-select></div><span>{{ visible.length }} 项保护措施 · {{ Object.keys(duplicateKeys).length }} 个独立键 · {{ suspendedCount }} 项暂停中</span></section>
    <section class="data-section">
      <el-table v-loading="store.loading" :data="visible" row-key="id" empty-text="暂无保护措施">
        ...
        <el-table-column label="暂停原因 / 替代办法" min-width="210"><template #default="{ row }"><div v-if="row.suspension_reason" class="primary-cell"><strong>{{ row.suspension_reason }}</strong><span>{{ row.compensating_measures }}</span></div><span v-else>—</span></template></el-table-column>
        <el-table-column label="计划恢复" min-width="150"><template #default="{ row }"><template v-if="row.lifecycle_state === 'suspended'"><span class="date-cell" :class="{ expired: row.restore_overdue }">{{ dateLabel(row.planned_restore_at) }}</span><small class="cell-note">{{ row.restore_overdue ? '已逾期未恢复' : '计划恢复日期' }}</small></template><span v-else-if="row.resumed_at" class="cell-note">已于 {{ dateLabel(row.resumed_at) }} 恢复</span><span v-else>—</span></template></el-table-column>
    ...
    <el-dialog v-model="suspendDialog" title="临时停用保护层" width="min(560px, 94vw)">
      <el-alert type="warning" :closable="false" show-icon title="暂停期间新运行的覆盖评估不会把该层计入保护；已生成的评估保持原评分与快照。" />
      <el-form label-position="top" @submit.prevent="submitSuspend">
        <el-form-item label="暂停原因" required><el-input v-model="suspendForm.reason" type="textarea" :rows="2" placeholder="例如：安全阀现场拆检" /></el-form-item>
        <el-form-item label="替代办法" required><el-input v-model="suspendForm.compensating_measures" type="textarea" :rows="2" placeholder="暂停期间的补偿措施，例如每班两次人工巡检并监视压力" /></el-form-item>
        <el-form-item label="计划恢复日期" required><el-date-picker v-model="suspendForm.planned_restore_at" type="datetime" value-format="YYYY-MM-DDTHH:mm:ssZ" :disabled-date="disablePastDate" /></el-form-item>
      </el-form>
      <template #footer><el-button @click="suspendDialog = false">取消</el-button><el-button type="warning" :loading="saving" @click="submitSuspend">确认暂停</el-button></template>
    </el-dialog>
    <el-dialog v-model="resumeDialog" title="复核恢复保护层" width="min(560px, 94vw)">
      <el-alert type="info" :closable="false" show-icon title="恢复后系统按本次验证时间把该层重新纳入后续评估；未提供验证证据将无法恢复。" />
      <el-form label-position="top" @submit.prevent="submitResume">
        <el-form-item label="验证时间" required><el-date-picker v-model="resumeForm.verified_at" type="datetime" value-format="YYYY-MM-DDTHH:mm:ssZ" /></el-form-item>
        <el-form-item label="验证证据" required><el-input v-model="resumeForm.evidence_note" type="textarea" :rows="3" placeholder="回装与功能试验证据，例如报告编号与存放位置" /></el-form-item>
      </el-form>
      <template #footer><el-button @click="resumeDialog = false">取消</el-button><el-button type="primary" :loading="saving" @click="submitResume">确认恢复</el-button></template>
    </el-dialog>
